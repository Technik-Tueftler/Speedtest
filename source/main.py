...
# -*- coding: utf-8 -*-
"""
Main function for Speedtest provides timer and function for threading the measurements
"""
import time
import os
...
class Timer:
    ...
    :param _runtime: Time how long the timer runs
    :param _start_time: Start time of the timer
    :param _timer_run: Information if timer is running
    """
    _runtime: float = 0
    ...
    def runtime(self) -> float:
        """Returns the time how long the timer runs in total"""
        return self._runtime

    @runtime.setter
    def runtime(self, value: float) -> None:
        """Sets the time how long the timer runs in total with a validation check
...
def run_timer(communication_interface: dict) -> None:
    """Scheduling function to process the timer running and to pause until the next event"""
    time_handler = Timer()
...
def overrun_timer() -> None:
    """Extend function to run a additional timer for 10 seconds"""
    time_handler = Timer()
...
def check_low_network_load(communication_interface: dict) -> None:
    """Function to check if current network load, which come from the fritzbox,
    exceed the set limits. If this is the case, no measurement is started in order not to
    load the network."""
    thr_network_upload_to_run = communication_interface["MBIT_THR_FROM_NETWORK_UPLOAD_TO_RUN"]
    thr_network_download_to_run = communication_interface["MBIT_THR_FROM_NETWORK_DOWNLOAD_TO_RUN"]
    ...
    loop_count = 0
    fritzbox_connector = communication_interface["fritzbox_connector"]
    while timer.timer_run:
        upstream_raw, downstream_raw = fritzbox_connector.transmission_rate
    ...
    if current_upstream_raw_in_mbit > thr_network_upload_to_run:
        check_is_ok &= False
    ...
    if current_downstream_raw_in_mbit > thr_network_download_to_run:
        check_is_ok &= False
...
def receive_network_load_from_fritzbox(communication_interface: dict) -> None:
    """Measurement thread to read the current network data from the fritzbox"""
    total_downstream_raw = []
    total_upstream_raw = []
    fritzbox_connector = communication_interface["fritzbox_connector"]
...
def measure_connection_speed(communication_interface: dict) -> None:
    """Measurement thread to start a internet speedtest via external API"""
    download = 0
    upload = 0
    ping = 0
    try:
        ...
        timer_thread = thread.Thread(target=overrun_timer)
        timer_thread.start()
        timer_thread.join()
        communication_interface["measurement_successful"] = True
    except speedtest.SpeedtestBestServerFailure as err:
        print(f"ERROR: No connection to the speedtest server possible. "
              f"Aborted with error: [{err}].")
...
def main(env_data: dict) -> None:
    """Function to scheduling the app"""
    ...
    while True:
        print("--------------------------------------------------")
        timer_thread = thread.Thread(target=run_timer, args=(communication,))
        timer_thread.start()
        timer_thread.join()

        network_check_thread = thread.Thread(target=check_low_network_load, args=(communication,))
        ...
        if not communication["low_stream_rate"]:
            ...
            measure_network_load_thread = thread.Thread(target=receive_network_load_from_fritzbox,
                                                        args=(communication,))
            receive_network_load_thread = thread.Thread(target=measure_connection_speed,
                                                        args=(communication,))
...
def print_speed_results(start_text: str, download_bits: int, upload_bits: int) -> None:
    """Print function to show the test results in the console"""
...
def check_and_verify_env_variables() -> dict:
    ...
    try:
        ...
        environment_data["all_verified"] &= False
        print("ERROR: The value for the variable S_TIME_CHECK_LOW_NETWORK_LOAD is not an integer")

    try:
        thr_network_download_to_run = int(os.getenv('MBIT_THR_FROM_NETWORK_DOWNLOAD_TO_RUN', "10"))
        environment_data["MBIT_THR_FROM_NETWORK_DOWNLOAD_TO_RUN"] = thr_network_download_to_run
        environment_data["all_verified"] &= True
    except ValueError as _:
        environment_data["all_verified"] &= False
        print("ERROR: The value for the variable MBIT_THR_FROM_NETWORK_DOWNLOAD_TO_RUN is not "
              "an integer")

    try:
        thr_network_upload_to_run = int(os.getenv('MBIT_THR_FROM_NETWORK_UPLOAD_TO_RUN', "2"))
        environment_data["MBIT_THR_FROM_NETWORK_UPLOAD_TO_RUN"] = thr_network_upload_to_run
        environment_data["all_verified"] &= True
    except ValueError as _:
        environment_data["all_verified"] &= False
        print("ERROR: The value for the variable MBIT_THR_FROM_NETWORK_UPLOAD_TO_RUN "
              "is not an integer")
    ...
    try:
        ...
        environment_data["all_verified"] &= False
        print("ERROR: The value for the variable TEST_REPETITION_TIME "
              "is not an integer")
